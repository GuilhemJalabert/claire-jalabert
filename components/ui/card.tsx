  ...
  "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl py-(--card-spacing) text-sm text-card-foreground transition-[box-shadow,transform,border-color,background-color] duration-[var(--duration-fast)] ease-[var(--ease-out)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl",
  {
    variants: {
      variant: {
        ...
        glass:
          "surface-glass rounded-[1.5rem] ring-1 ring-border/35 [--card-spacing:--spacing(5)] sm:rounded-[1.75rem] sm:[--card-spacing:--spacing(7)]",
        interactive:
          "bg-card shadow-soft ring-1 ring-border/50 hover:-translate-y-0.5 hover:shadow-elevated hover:ring-luminous/30",
        editorial:
          "bg-card ring-1 ring-border/40 [--card-spacing:--spacing(5)] sm:[--card-spacing:--spacing(7)]",
      },
      size: {
